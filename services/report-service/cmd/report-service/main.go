...
func main() {
	...
	pagosRepo := repository.NewPagosRepository(pagosPool)
	prestamosRepo := repository.NewPrestamosRepository(prestamosPool)
	...
	r.GET("/ready", func(c *gin.Context) {
		...
		for name, p := range map[string]interface{ Ping(context.Context) error }{
			...
			if err := p.Ping(pingCtx); err != nil {
				c.JSON(http.StatusServiceUnavailable, gin.H{"status": "down", "db": name, "error": err.Error()})
				return
			}
		}
	...
	reportHandler.Register(api.Group("/reports"))
